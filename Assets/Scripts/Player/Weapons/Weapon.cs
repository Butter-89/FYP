...
public class Weapon : NetworkBehaviour {
    ...
    public Auto auto;
    public GameObject weaponModel;//not applicable here
    ...
    private void Fire()
    {
        ...
        for (int i = 0; i < shotPerRound; i++)
        {
            ...
            if (Physics.Raycast(ray, out hit, range))
            {
                ...
                if (castle != null)
                {
                    _player.CmdHitCastle(damage, castle.netId);
                    //Debug.Log(castle.netId);
                }
    ...
    private void Launch()
    {
        ...
        // Subtract 1 from the current ammo
        _currentAmmo--;
        var position = raycastStartSpot.transform.position;
        var Px = position.x;
        var Py = position.y;
        var Pz = position.z;
    ...
    {
        if(projectile==null)
            Debug.Log("No projectile is attached");
        var newProjectile = Instantiate(projectile, raycastStartSpot.transform.position, raycastStartSpot.rotation);
